...
function TokenImage({ symbol, size = "md", className = "" }: TokenImageProps) {
  ...
  if (imageError) {
    ...
    return (
      <div
        className={`${sizeClasses[size]} ${className} bg-gray-300 rounded-full flex items-center justify-center`}
      >
        <span className="text-gray-700 text-xs font-bold">{symbol.slice(0, 2).toUpperCase()}</span>
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="w-full bg-white hover:bg-gray-50 border border-gray-300 rounded-lg px-4 py-3 text-left transition-colors shadow-sm"
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <TokenImage symbol={selectedToken} size="md" />
            <div>
              <div className="text-gray-900 font-medium">{selectedToken}</div>
              {selectedTokenInfo && (
                <div className="text-xs text-gray-600">{selectedTokenInfo.name}</div>
              )}
            </div>
          </div>
          <svg
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-xl z-50 max-h-96 flex flex-col">
          {/* Network indicator */}
          <div className="px-3 py-2 text-xs text-gray-600 border-b border-gray-200 bg-gray-50">
          ...
          <div className="p-3 border-b border-gray-200">
            <input
              ...
              className="w-full bg-gray-50 border border-gray-300 rounded px-3 py-2 text-gray-900 placeholder-gray-500 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
            />
          </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div
                            className={`text-xs ${
                              selectedToken === token.symbol ? "text-blue-100" : "text-gray-600"
                            }`}
                          >
                            {token.name}
                          </div>
